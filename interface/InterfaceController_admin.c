...
static void adminRemovePeer(Dict* args, void* vcontext, String* txid)
{
    struct Context* context = vcontext;
    struct Allocator* alloc = Allocator_child(context->alloc);
    ...
    if (error) {
      ...
      error = context->ic->removePeer(context->ic,pubkey);
      if (error) {
                                        ...
                                        struct Allocator* alloc)
{
    ...
    Admin_registerFunction("InterfaceController_removePeer", adminRemovePeer, ctx, true,
        ((struct Admin_FunctionArg[]) {
